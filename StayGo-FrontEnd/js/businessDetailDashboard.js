const backendUrl = "http://localhost:8080";
    ...
    async function loadBusinessOrders() {
        ...
        $.ajax({
            method: "GET",
            url: `${backendUrl}/api/v1/bookings/business/${businessId}`,
            headers,
            success: function(response) {
                const container = $("#business-orders").empty();
                if (!response.data || response.data.length === 0) {
                    container.append(`<p class="text-muted">No bookings found.</p>`);
                    return;
                }

                response.data.forEach(b => {
                    const checkIn = new Date(b.checkInTime).toLocaleString();
                    const checkOut = new Date(b.checkOutTime).toLocaleString();
                    ...
                    container.append(`
                        <div class="booking-card">
                            <h5>Booking #${randomPrefix}-${b.bookingId}</h5>
                            <p><strong>${b.firstName} ${b.lastName}</strong> | ${b.email}</p>
                            <p>${b.phoneNumber} | ${b.address}</p>
                ...
                response.data.forEach(d => {
                    container.append(`
                        <div class="col-md-4 mb-4">
                            <div class="card">
                                <img src="${d.roomImage ? backendUrl + '/' + d.roomImage : '/images/default-logo.png'}" class="card-img-top" style="height:200px; object-fit:cover;">
                                <div class="position-absolute p-2 top-0 end-0 m-2 d-flex gap-2">
                                    <button class="btn btn-sm btn-primary edit-business" data-id="${d.businessDetailId}" title="Edit">
                                        <i class="bi bi-pencil-square"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger delete-business" data-id="${d.businessDetailId}" title="Delete">
                                        <i class="bi bi-trash"></i>
                                    </button>
                                </div>
                                <div class="card-body">
                                    <h5 class="text-primary">${d.luxuryLevel}</h5>
                                    <p>Rooms: ${d.roomsCount}</p>
                                    <p>Beds per room: ${d.bedsCount}</p>
                                    <p>Guest Count per room: ${d.guestCount}</p>
                                    <p>Price Per Day: LKR ${d.pricePerDay}</p>
                                    <p>Price Per Night: LKR ${d.pricePerNight}</p>
                                    <h6 class="badge w-50 p-2 bg-${d.status === 'ACTIVE' ? 'success' : 'danger'}">${d.status}</h6>
                                </div>
                            </div>
                        </div>
                    `);
    ...
    loadBusinessOrders();  // load bookings
    loadBusinessDetails(); // load business details
